        ...
        const extractTextFromElement = (element: React.ReactNode) => {
            ...
            if (React.isValidElement(element)) {
                ...
                if (element.props.children.length > 1) {
                    for (let i = 0; i < element.props.children[2].props.children.length; i++) {
                        const subMenuLink = {
                            href: element.props.children[2].props.children[i].props.children[0].props.href,
                            text: element.props.children[2].props.children[i].props.children[0].props.children[0]
                        }
            ...
            onMouseLeave={() => handleMouseLeave(menuItem.text)} className="relative">
              ...
              <div className="hidden md:block">
                <div style={{ display: menuItem.isShown ? "block" : "none" }} className="absolute left-0 bg-blue-950 rounded-md shadow-lg min-w-full text-center pt-4 pb-1 hidden md:inline-block">
                  {menuItem.subMenuItems.map((subMenuItem, subIndex) => (
                    <a key={subIndex} href={subMenuItem.href} className="block px-4 py-2 text-sky hover:bg-blue-900">
                      {subMenuItem.text}
                    </a>
                  ))}
                </div>
              ...
              <div className="block md:hidden">
                <div style={{ display: menuItem.isShown ? "block" : "none" }}>
                  {menuItem.subMenuItems.map((subMenuItem, subIndex) => (
                    <a key={subIndex} href={subMenuItem.href} className="block px-4 py-2 text-sky hover:bg-blue-900 text-center">
                      {subMenuItem.text}
                    </a>
                  ))}
                </div>
